  ...
  mit.ForkUtils = {
    ...
    */
    ...
    create: function () {
      ...
      if (forks.length < count) {
        ...
        for (var i = 0; i < count - forks.length; i++) {
          ...
          // Height and Width
          fork.w = fork_img.width / 2;
          fork.h = fork_img.height / 2;
    ...
    draw: function (ctx) {
      ...
      forks.forEach(function (fork, index) {
        ...
        if (fork.edge === 'top') {
          ...
          ctx.translate(~~(fork_img.width / 4), ~~(fork_img.height / 4));
          ctx.rotate(utils.toRadian(180));
          ctx.drawImage(fork_img, -~~(fork_img.width / 4), -~~(fork_img.height / 4), fork_img.width / 2, fork_img.height / 2);
          ctx.restore();


          fork.head_x = fork.x - ~~(fork_head_img.width / 16);
          fork.head_y = fork.y + fork_img.height / 2;

          fork.head_w = fork_head_img.width / 2;
          fork.head_h = fork_head_img.height / 2;

          // Draw Fork Head
          ctx.save();
          ctx.translate(fork.head_x, fork.head_y);
          ctx.translate(~~(fork_head_img.width / 4), ~~(fork_head_img.height / 4));
          ctx.rotate(utils.toRadian(180));
          ctx.drawImage(fork_head_img, -~~(fork_head_img.width / 4), -~~(fork_head_img.height / 4), fork_head_img.width / 2, fork_head_img.height / 2);
          ctx.restore();
        } else if (fork.edge === 'btm') {

          ctx.drawImage(fork_img, fork.x, fork.y, fork_img.width / 2, fork_img.height / 2);

          fork.head_x = fork.x - ~~(fork_head_img.width / 10);
          fork.head_y = fork.y - fork_head_img.height / 2;
          ...
          ctx.save();
          ctx.translate(fork.head_x, fork.head_y);
          ctx.translate(1 * ~~(fork_head_img.width / 4), 1 * ~~(fork_head_img.height / 4));
          ctx.scale(-1, 1);
          ctx.drawImage(
            fork_head_img,
            1 * -~~(fork_head_img.width / 4) - 2,
            1 * -~~(fork_head_img.height / 4),
            fork_head_img.width / 2,
            fork_head_img.height / 2
          );
          ctx.restore();
        }
    ...
    checkCollision: function () {
      ...
      if (
        pappu_bounds.end_x > fork_head_bounds.start_x + 40 &&
        fork_head_bounds.end_x - 40 > pappu_bounds.start_x &&
        pappu_bounds.end_y > fork_head_bounds.start_y + 40 &&
        fork_head_bounds.end_y - 40 > pappu_bounds.start_y
      ) {
        mit.gameOver();
      }
    }
